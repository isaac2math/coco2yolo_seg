...
def ImportCoco(path, path_to_images=None, name=None, encoding='utf-8', seg_only=False):
    """
    ...
    if seg_only == False:
        ...
        objs = pd.json_normalize(annotations_json["annotations"])
        num_objs = objs.shape[0]

        df[["ann_bbox_xmin", "ann_bbox_ymin", "ann_bbox_width", "ann_bbox_height"]] = pd.DataFrame(np.zeros((num_objs, 4)))
